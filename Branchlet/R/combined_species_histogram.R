## Combined histogram: Eucalyptus, Acacia, Pine
...
df_euc <- read_excel(excel_path, sheet = "twigs (2)") %>% mutate(condition = "Eucalyptus")
df_acacia <- read_excel(excel_path, sheet = "Acacia") %>% mutate(condition = "Acacia")
df_pine <- read_excel(excel_path, sheet = "Pine") %>% mutate(condition = "Pine")

## Combine all three
df_all <- bind_rows(df_euc, df_acacia, df_pine)

## Compute vol/sa ratio
df_all <- df_all %>%
    mutate(
        vol_sa_ratio = `Volume (mm3)` / `Surface Area (mm2)`,
        condition = factor(condition, levels = c("Eucalyptus", "Acacia", "Pine"))
    )
...
out_file <- paste0(figures_dir, "/Combined_species_vol_sa_ratio_histogram.png")
