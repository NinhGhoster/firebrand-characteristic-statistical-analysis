...
As requested, we conducted a Pearson correlation analysis across all variables (**count**, **length**, **width**, **height**, **mass**, **surface area**, **volume**, and **V/Sa**) for all three fuel types. To appropriately include the `count` variable alongside individual physical properties, the data was aggregated (averaged) by experiment/trunk section so that all metrics align on the same scale.
...
3. **V/Sa**: Defines the *shape/compactness*. It captures the thickness, aerodynamics, and burning lifetime of the particle, which raw mass cannot explain alone.
...
**Figure X.** Pearson correlation matrices of physical and geometric parameters (count, length, width, height, mass, surface area, volume, and V/Sa) for Branchlet, Stringybark, and Candlebark firebrands. Data were aggregated by experimental run and trunk section to align scale across variables. The correlation coefficient (r) is represented by the color scale (value index), ranging from -1 (strong negative correlation, typically shown in blue/cool colors) to 1 (strong positive correlation, typically shown in red/warm colors), with 0 indicating no linear relationship. Asterisks denote the statistical significance of the correlations (* p < 0.05, ** p < 0.01, *** p < 0.001). The matrices illustrate high redundancy among primary size dimensions (length, width, height) and mass-based metrics, whereas the firebrand count and V/Sa exhibit greater independence, capturing distinct mechanistic properties of firebrand generation and aerodynamics.
